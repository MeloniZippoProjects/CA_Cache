...
# DATA
    ...
    # Technology nodes
        $techFolderNames = @("32nm");
        $techSettingLine = "-technology (u) ";
        $techSettingValues = @("0.032");
...
# EXECUTION
    ...
    $baseContent = Get-Content -Path $baseConfig;
    New-Item -type directory -Force $cfgFolder | Out-Null;
    $origin = pwd;
    cd $cfgFolder;
    ...
    for( $techIndex = 0; $techIndex -lt $techFolderNames.length; $techIndex++)
    {
        ...
        New-Item $techFolderNames[$techIndex] -type directory -Force | Out-Null;
    $techLevelDir = pwd;
        ...
        for( $typeIndex = 0; $typeIndex -lt $typeSettingValues.length; $typeIndex++)
        {
            ...
            New-Item $typeFolderNames[$typeIndex] -type directory -Force | Out-Null;
            $typeLevelDir = pwd;
